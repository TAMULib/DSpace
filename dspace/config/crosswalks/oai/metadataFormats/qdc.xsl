...
<xsl:stylesheet 
	...
	<xsl:template match="/">
		<qdc:qualifieddc xmlns:qdc="http://dspace.org/qualifieddc/"
				...
				xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://purl.org/dc/elements/1.1/ http://dublincore.org/schemas/xmls/qdc/2006/01/06/dc.xsd http://purl.org/dc/terms/ http://dublincore.org/schemas/xmls/qdc/2006/01//dcmi/xmlschema/qualifieddc.xsd">
			...
			<!-- dc.contributor.* (other than author, advisor, editor, illustrator, committeechair, committeeMember, photographer, sponsor, or other) -->
			<xsl:for-each select="doc:metadata/doc:element[@name='dc']/doc:element[@name='contributor']/doc:element[@name!='author'][@name!='editor'][@name!='illustrator'][@name!='committeechair'][@name!='committeeMember'][@name!='photographer'][@name!='sponsor'][@name!='other']/doc:element/doc:field[@name='value']">
				<dc:contributor>
					<xsl:value-of select="." />
				</dc:contributor>
			</xsl:for-each>
			...
			<!-- dc.identifier.* (excluding citation, govdoc, isbn, issn, sici, uri) -->
			<xsl:for-each select="doc:metadata/doc:element[@name='dc']/doc:element[@name='identifier']/doc:element[@name!='citation'][@name!='govdoc'][@name!='isbn'][@name!='issn'][@name!='sici'][@name!='uri']/doc:element/doc:field[@name='value']">
				<dc:identifier>
					<xsl:value-of select="." />
				</dc:identifier>
			</xsl:for-each>
			...
			<!-- dc.relation.* (excluding isformatof, ispartof, haspart, isversionof, hasversion, ispartofseries, isreferencedby, requires, replaces, isreplacedby, uri)-->
			<xsl:for-each select="doc:metadata/doc:element[@name='dc']/doc:element[@name='relation']/doc:element[name!='isformatof'][name!='ispartof'][name!='haspart'][name!='isversionof'][name!='hasversion'][name!='ispartofseries'][name!='isreferencedby'][name!='requires'][name!='replaces'][name!='isreplacedby'][name!='uri']/doc:element/doc:field[@name='value']">
				<dc:relation>
					<xsl:value-of select="." />
				</dc:relation>
			</xsl:for-each>
			...
			<!-- *********************************** -->			
			<!-- TAMU Specific customizations follow -->
			<!-- *********************************** -->
			...
			<!-- TAMU secific: thesis.degree.grantor -->
			<xsl:for-each select="doc:metadata/doc:element[@name='thesis']/doc:element[@name='degree']/doc:element[@name='grantor']/doc:element/doc:field[@name='value']">
				<thesis:degree xmlns:thesis="http://digital.library.tamu.edu/schemas/thesis" type="grantor">
					<xsl:value-of select="." />
				</thesis:degree>
			</xsl:for-each>
